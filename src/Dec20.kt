object Dec20 {
    ...
    fun a(): Int {
        ...
        for (i in input.indices) {
            nodes.add(Node(i, input[i].toInt()))
        }
        ...
        for (i in nodes.indices) {
            ...
            var newIndex = (currentIndex + node.number)
...
//            println("After  (${node.number}): " + output.map { it.number })
        ...
        val indexOfZero = output.indexOf(output.find { it.number == 0 })
        var sum = 0
        for (i in 1..3) {
    ...
    private class Node(val id: Int = 0, val number: Int = 0)
}